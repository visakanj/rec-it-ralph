      ...
      <div className="relative bg-surface p-5 rounded-2xl border border-border-subtle shadow-subtle transition-all duration-300 active:scale-[0.98] hover:border-border-highlight">
        <div className="flex justify-between items-start mb-4">
          <div className="flex items-center space-x-2">
            <div className={`w-2 h-2 rounded-full ${active ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]' : 'bg-text-tertiary'}`} />
            <span className="text-xs font-medium text-text-tertiary uppercase tracking-wider">
              {active ? 'Active Now' : lastActive || 'Inactive'}
            </span>
          </div>
          <ChevronRight size={20} className="text-text-tertiary opacity-50 group-hover:opacity-100 transition-opacity" />
        </div>
